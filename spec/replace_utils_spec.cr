...
describe Adamantine::ReplaceUtils do
  describe ".parse_replace_arguments" do
    it "parses default replace command" do
      result = Adamantine::ReplaceUtils.parse_replace_arguments("/old/new/")
    ...
    it "parses flags" do
      result = Adamantine::ReplaceUtils.parse_replace_arguments("/old/new/ gic")
      raise "expected parsed arguments" unless result
      old_text, new_text, flags = result
      raise "wrong flags" unless flags.global && flags.ignore_case && flags.preview
    end
    ...
    it "rejects unsupported flags" do
      result = Adamantine::ReplaceUtils.parse_replace_arguments("/old/new/x")
      raise "expected nil" unless result.nil?
    end

    it "handles escaped delimiter" do
      result = Adamantine::ReplaceUtils.parse_replace_arguments("/old\\/old/new/")
      raise "expected parsed arguments" unless result
      old_text, new_text, flags = result
    ...
    it "accepts empty replacement text" do
      result = Adamantine::ReplaceUtils.parse_replace_arguments("/old//g")
      raise "expected parsed arguments" unless result
      old_text, new_text, flags = result
      raise "wrong old text" unless old_text == "old"
  ...
  describe ".replace_text_content" do
    it "replaces first occurrence by default" do
      ...
      replaced = Adamantine::ReplaceUtils.replace_text_content("a b a b", "a", "x", flags)
    ...
    it "ignores case when requested" do
      flags = Adamantine::ReplaceUtils::ReplaceFlags.new(ignore_case: true, global: true)
      replaced = Adamantine::ReplaceUtils.replace_text_content("Ab c aB", "ab", "x", flags)
      raise "wrong replaced" unless replaced == "x c x"
    end

    it "handles escaped pattern as plain text" do
      flags = Adamantine::ReplaceUtils::ReplaceFlags.new(global: true)
      replaced = Adamantine::ReplaceUtils.replace_text_content("a/b a/b", "a/b", "x", flags)
  ...
  describe ".replace_match_count" do
    it "counts occurrences with global flag and ignore case" do
      ...
      count = Adamantine::ReplaceUtils.replace_match_count("AbAb", "ab", flags)
  ...
  describe ".make_replace_previews" do
    it "reports line and columns for matches" do
      text = "one foo two\nfoo three"
      flags = Adamantine::ReplaceUtils::ReplaceFlags.new
      output = Adamantine::ReplaceUtils.make_replace_previews(text, "foo", "bar", 2, flags)
    ...
    it "handles long lines with truncation" do
      text = "#{"a" * 120}\n"
      flags = Adamantine::ReplaceUtils::ReplaceFlags.new
      output = Adamantine::ReplaceUtils.make_replace_previews(text, "a", "x", 1, flags)
    ...
    it "highlights unicode safely" do
      text = "😀 old ✅\n"
      flags = Adamantine::ReplaceUtils::ReplaceFlags.new
      output = Adamantine::ReplaceUtils.make_replace_previews(text, "old", "new", 10, flags)
    ...
    it "respects preview limit" do
      ...
      flags = Adamantine::ReplaceUtils::ReplaceFlags.new(global: true)
      output = Adamantine::ReplaceUtils.make_replace_previews(text, "old", "x", 2, flags)
  ...
  describe ".flags_to_label" do
    it "returns readable labels for all flag combos" do
      raise "wrong single label" unless Adamantine::ReplaceUtils.flags_to_label(Adamantine::ReplaceUtils::ReplaceFlags.new) == "single"
      raise "wrong ignore label" unless Adamantine::ReplaceUtils.flags_to_label(Adamantine::ReplaceUtils::ReplaceFlags.new(ignore_case: true)) == "first (ignore case)"
      raise "wrong global label" unless Adamantine::ReplaceUtils.flags_to_label(Adamantine::ReplaceUtils::ReplaceFlags.new(global: true)) == "all (global)"
      raise "wrong all ignore label" unless Adamantine::ReplaceUtils.flags_to_label(Adamantine::ReplaceUtils::ReplaceFlags.new(global: true, ignore_case: true)) == "all (global, ignore case)"
    end
  end
end
