...
describe Adamantine::SemanticOverlay do
  it "returns nil for empty overlay" do
    overlay = Adamantine::SemanticOverlay.empty
  ...
  it "decodes a single delta-encoded token" do
    ...
    data = [0, 0, 3, 15, 0]
    overlay = Adamantine::SemanticOverlay.build(data, ["def foo"], Adamantine::SemanticOverlay::STANDARD_LEGEND)
  ...
  it "decodes two tokens on the same line with relative start" do
    ...
    overlay = Adamantine::SemanticOverlay.build(data, ["x = 42"], Adamantine::SemanticOverlay::STANDARD_LEGEND)
  ...
  it "decodes tokens on a later line using deltaLine" do
    data = [1, 2, 3, 18, 0]
    overlay = Adamantine::SemanticOverlay.build(data, ["", "  foo"], Adamantine::SemanticOverlay::STANDARD_LEGEND)
  ...
  it "ignores trailing incomplete quintuples" do
    data = [0, 0, 3, 15, 0, 1, 0]
    overlay = Adamantine::SemanticOverlay.build(data, ["def"], Adamantine::SemanticOverlay::STANDARD_LEGEND)
    raise "complete token should still apply" unless overlay.name_at(0, 0) == "keyword"
  end

  it "clamps token length to the line" do
    data = [0, 0, 50, 15, 0]
    overlay = Adamantine::SemanticOverlay.build(data, ["ab"], Adamantine::SemanticOverlay::STANDARD_LEGEND)
    raise "col 0 should be keyword" unless overlay.name_at(0, 0) == "keyword"
  ...
  it "reports tokens after decode" do
    overlay = Adamantine::SemanticOverlay.build([0, 0, 3, 15, 0], ["def"], Adamantine::SemanticOverlay::STANDARD_LEGEND)
  ...
  it "fills Crystal hash comments outside strings" do
    ...
    overlay = Adamantine::SemanticOverlay.build(data, [%("hello" # note)], Adamantine::SemanticOverlay::STANDARD_LEGEND)
  ...
  it "does not treat # inside a string token as a comment" do
    overlay = Adamantine::SemanticOverlay.build([0, 0, 9, 18, 0], [%("a#b#cde")], Adamantine::SemanticOverlay::STANDARD_LEGEND)
...
describe Adamantine::Lsp::SemanticTokens do
  ...
  it "returns empty for nil or null result" do
    raise "nil should be empty" unless Adamantine::Lsp::SemanticTokens.parse_data(nil).empty?
    raise "null should be empty" unless Adamantine::Lsp::SemanticTokens.parse_data(JSON.parse("null")).empty?
  end

  it "returns empty for missing data field" do
    raise "missing data should be empty" unless Adamantine::Lsp::SemanticTokens.parse_data(JSON.parse(%({"resultId":"v1"}))).empty?
  end
end

describe Adamantine::Theme::Syntax do
  it "uses vscode-dark keyword color after reset" do
    Adamantine::Theme.reset
    color = Adamantine::Theme::Syntax.color("keyword")
  ...
  it "applies token color without replacing background" do
    ...
    styled = Adamantine::Theme::Syntax.apply(base, "string")
  ...
  it "keeps base style when token type is unknown" do
    ...
    styled = Adamantine::Theme::Syntax.apply(base, "not-a-token")
  ...
  it "loads syntax overrides from JSON" do
    ...
    begin
      ...
      Adamantine::Theme.reset
      raise "load should succeed" unless Adamantine::Theme.load(theme_file.to_s)
      raise "keyword override should apply" unless Adamantine::Theme::Syntax.color("keyword") == Tui::Color.rgb(255, 0, 170)
      raise "string should keep preset" unless Adamantine::Theme::Syntax.color("string") == Tui::Color.rgb(206, 145, 120)
    ensure
      FileUtils.rm_rf(tmp)
    end
  end
  ...
  it "switches syntax palette with light preset" do
    ...
    raise "light keyword should be blue" unless Adamantine::Theme::Syntax.color("keyword") == Tui::Color.rgb(0, 0, 255)
  end
end
